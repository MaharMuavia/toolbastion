...
5. In enforce mode, require an approved schema-validated capability contract for every tool. `filesystem: none` blocks filesystem requests, `filesystem: read` blocks writes and destructive operations, and `filesystem: write` requires existing project-relative Docker writable mounts. A declared network-denied, command, subprocess, or destructive capability requires the Docker no-network profile; `network: allowlist` fails closed until an authenticated egress proxy exists. Detectors are evidence only and cannot grant authority.
...
ToolBastion sees MCP messages, not TCP connections made by an arbitrary target. Capability declarations are therefore operator-reviewed trust data, hashed into the baseline; changing one invalidates trust and cached decisions. Baseline v4 binds trust to the inspected Docker image identity or a non-Docker executable plus entrypoint, package manifest, lockfile, and complete statically discoverable dependency closure. Docker isolation verifies that an immutable image is available and launches the target with `--network=none`, a read-only project mount beneath an image-owned runtime dependency directory, optional existing project-relative writable submounts, dropped capabilities, `no-new-privileges`, non-root UID, bounded tmpfs, and PID/memory/CPU limits. The container cannot resolve or connect to any network destination; this is containment, not an allowlisted egress proxy. The retained `network.target_egress` setting describes topology only and never grants a capability.

## Reliability boundary

The proxy and target transport do not depend on Fastify, React, or a browser. Closing the dashboard cannot stop enforcement. `toolbastion run` writes a separately redacted lifecycle JSONL file for local observation; the API reads it without participating in decisions. One bounded tailer serves every SSE client and reads only appended bytes after its initial load. Runtime segments rotate under `runtime_events`; a retained-boundary marker keeps the session identity explicit and the dashboard labels the resulting view `LIVE PARTIAL` rather than presenting incomplete lifecycle metrics as complete. Audit evidence uses exclusive v2 start/event/seal files; CLI and live-dashboard readers canonicalize the configured audit directory and session file beneath `project_root`, then consume one verified read rather than verify-then-reread. If no valid live log exists, the dashboard falls back to the verified fixture. The hosted snapshot contains static redacted artifacts and no API credentials.
