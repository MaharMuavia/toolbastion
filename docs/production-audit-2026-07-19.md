...
| Production readiness | 76/100 | Reproducible build, CI, hardened container execution, and local tests are verified; live OpenAI remains unavailable. |
...
| AI reliability | 68/100 | Inputs/outputs are bounded and structured; live GPT-5.6 acceptance is unverified because the configured account is inactive. |
...
| `packages/judge` | Partially working | OpenAI Responses integration uses structured parsing, timeouts, limits, and `store: false`; live acceptance was not available. |
...
| GPT semantic judgment | Three structured Responses subchecks | Structural request profile | Unit failure paths | Partially working | No successful live GPT-5.6 execution in this audit. |
...
| Docker target isolation | No-network, RO, non-root, resource-limited run | Docker runtime | CI configuration/test present | Unverified locally | Docker daemon unavailable. |
...
| AI-01 | High | Live model proof | Evaluation docs and live behavior show account-inactive HTTP 429; no successful live call was available. | Open: activate an OpenAI project and record a sanitized live structured-check proof. |
...
- [ ] Activate and verify the intended OpenAI project; save only sanitized proof of live structured checks.
...
1. Obtain one successful live GPT-5.6 structured-decision run and record sanitized latency/token evidence.
