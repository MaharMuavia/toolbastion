...
- External handoff required: configure/push a public repository, run Pages/release workflows, verify the uploaded YouTube demo and `/feedback` record, and complete the Devpost form
- Publication rule: repository links, hosted URL, image, video URL, and feedback ID are recorded; the release tag, image digest, and checksums remain pending until each destination is verified
- Packaging: release candidate version `0.1.4`; the GitHub tag, image digest, checksums, and publication gates remain pending until the release commit is tagged and verified
- Submission readiness: local engineering gates are implemented; remaining blockers are account-owned publication, live GPT-5.6 billing acceptance, and final Devpost submission fields
